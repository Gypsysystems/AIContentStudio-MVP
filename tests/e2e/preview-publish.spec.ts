...
test('real Preview renders all committed stable-ID topics and structures, selected style/layout/master, media and warnings after reload', async ({ page }) => {
  ...
  await page.reload()
  await page.getByRole('button', { name: 'Publish', exact: true }).click()
  await expect(page.getByTestId('publish-conditional-warning')).toContainText('HTML outputs include conditional blocks unfiltered')
  await page.getByRole('button', { name: 'Full Preview' }).click()
  ...
  await expect(preview.getByTestId('preview-conditional-export-warning')).toContainText('HTML export would include them unfiltered')
