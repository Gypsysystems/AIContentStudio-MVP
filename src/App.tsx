              ...
              <div className="flex gap-1.5 mt-3">
                ...
                {localProfiles.length > 1 && editProfile && (
                  <button onClick={() => {
                    ...
                    const remaining = storedProfiles.filter(p => p.id !== editId)
                    const fallback = remaining.find(p => p.clientId === activeThemeObj?.id) ?? remaining[0]
                    setLocalProfiles(remaining)
                    if (editId === activeStyleProfileId) {
                      onApplyStyleProfile(fallback?.id ?? '')
                    }
