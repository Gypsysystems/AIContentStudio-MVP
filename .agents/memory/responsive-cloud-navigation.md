...
Cloud section navigation may update the interface before a remote save finishes, but leave-project navigation and Review-to-Author transitions must retain save barriers. Development-mode local navigation should also retain its barrier. Opening and leaving project settings without edits must not advance the cloud record revision.
