...
**How to apply:** Before enabling shared or cloud-backed projects, introduce real trusted session and membership providers plus server-owned content storage and an explicit migration plan. A fixed local identity must remain restricted to loopback clients; a remotely reachable hosted preview requires real authentication even in development. Keep backup-as-new portable, authorize replacement against the destination, and make client operations fail rather than fall back to local permissions when the server gate is unavailable. Treat asynchronous authorization as a prerequisite for UI navigation and project activation.